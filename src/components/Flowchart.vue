...
// const a : d3.ArrayLike<d3.Selection<SVGLineElement, unknown, null, undefined>>
...
onMounted(() => {
  ...
  const grid = buildGrid(CANVAS_WIDTH, CANVAS_HEIGHT, GRID_SIZE)
  ...
  const updateLink = function (
    ...
    selection
      // .selectAll<SVGLineElement, LinkType>('line')
      .attr('x1', function (d: NodeType): number {
        // console.log(d)
        return d.source.screenX || 50
      })
      .attr('y1', function (d: LinkType): number {
        return d.source.screenY || 50
      })
      .attr('x2', function (d: LinkType): number {
        return d.target.screenX || 100
      })
      .attr('y2', function (d: LinkType): number {
        return d.target.screenY || 100
      })
  }
